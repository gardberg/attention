...
def test_conv1d_groups(in_channels, out_channels, kernel_size, stride, padding, groups):
    ...
    torch_conv = TorchConv1d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, groups=groups)
    with torch.no_grad():
        torch_out = torch_conv(x)
    torch_out = jnp.array(torch_out)
    print(f"torch_out.shape: {torch_out.shape}")

    jax_conv = Conv1d(in_channels, out_channels, kernel_size, stride, padding=padding, groups=groups)
    jax_state = to_jax_state(torch_conv)
    jax_out = jax_conv.forward(jax_state, jnp.array(x))

    assert torch_out.shape == jax_out.shape, f"Torch: {torch_out.shape}, Jax: {jax_out.shape}"
    assert jnp.allclose(torch_out, jax_out, atol=1e-5), f"Diff: {jnp.linalg.norm(torch_out - jax_out):.2e}"
