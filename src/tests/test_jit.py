...
def test_t5_forward_jit(t5_config):
    ...
    input_ids = jax.random.randint(jax.random.PRNGKey(0), (BATCH_SIZE, SEQ_LEN), 0, t5_config.vocab_size)
    decoder_input_ids = jax.random.randint(jax.random.PRNGKey(0), (BATCH_SIZE, SEQ_LEN), 0, t5_config.vocab_size)

    rng = jax.random.PRNGKey(0)

    jax_t5_model.set_use_jit(False)
    logits_nojit, enc_out_nojit = jax_t5_model.forward(state, input_ids, decoder_input_ids, rng)

    jax_t5_model.set_use_jit(True)
    logits_jit, enc_out_jit = jax_t5_model.forward(state, input_ids, decoder_input_ids, rng)
...
def test_benchmark_t5_forward(t5_config, benchmark, use_jit: bool):
        ...
        output_no_jit = benchmark(jax_t5_model.forward, state, input_ids, decoder_input_ids, rng)
